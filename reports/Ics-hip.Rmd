...
```{r}
buscas_ic %>% 
    boot(statistic = theta_diferenca_ab, R = 1000) %>% 
...
# }
# 
# theta_embaralhado(buscas_ic)
# ```

# ```{r}
# diffs = replicate(5000, {theta_embaralhado(buscas_ic)})
# mean(abs(diffs) >= abs(theta_diferenca_ab(buscas_ic, 1:NROW(buscas_ic))))
# ```