...
```{r}
clicks_aux = buscas %>% select(session_id, session_start_date, group, num_clicks) %>%
    mutate(day_date = floor_date(session_start_date, unit = "day"))

#glimpse(clicks_per_day)
```
```{r}
buscas %>% 
...
zeros_per_week %>% filter(group == "a") %>%
    ...
    geom_col(data = zeros_per_week, mapping = aes(y = zeros_results_day, fill = "Zero resultados")) +      geom_text(aes(label = zeros_results_day, y = zeros_results_day), size = 3) +
