...
session_id_clicks <- buscas %>%
    ...
    select(session_id, group, num_clicks, day_date) %>%
    summarise(nclicks = sum(num_clicks))
    #ungroup()
    glimpse(session_id_clicks)

session_id_clicks2 <- session_id_clicks %>%
    group_by(day_date, group) %>%
    summarise(num_sessions = sum(!is.na(session_id)),
              zero_clicks = sum(num_clicks == 0),
              gt_zero_clicks = sum(num_clicks > 0))
